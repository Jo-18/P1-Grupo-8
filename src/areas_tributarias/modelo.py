...
class ModeloGeometria:
    ...
    def __init__(self):
        ...
        self.notas: List[str] = []
        # Secciones del contrato que se conservan sin calcular (contrato Unity y
        # otras herramientas). No se ejecuta calculo estructural sobre ellas.
        self.fuente: dict = {}
        self.completitud_geometrica: str = ""
        self.columnas_referencia: list = []
        self.aberturas_globales: list = []
        self.juntas_dilatacion: list = []
        self.interfaces_junta: list = []
        self.pendientes: list = []
        self.validacion: dict = {}
    ...
    def from_dict(cls, data: dict) -> "ModeloGeometria":
        ...
        # Secciones conservadas del contrato (sin calcular; solo retencion).
        m.fuente = data.get("fuente", {})
        m.completitud_geometrica = data.get("completitud_geometrica", "")
        # 'columnas_referencia' es la clave canonica; 'columnas' se admite solo
        # como alias legado. Si existen ambas y difieren, se reporta error.
        cols_canonico = data.get("columnas_referencia")
        cols_legado = data.get("columnas")
        if cols_canonico is not None and cols_legado is not None:
            if cols_canonico != cols_legado:
                raise ValueError(
                    "conflicto de columnas: 'columnas_referencia' y 'columnas' "
                    "estan presentes y difieren; indicar una sola clave")
            m.columnas_referencia = cols_canonico
        elif cols_canonico is not None:
            m.columnas_referencia = cols_canonico
        elif cols_legado is not None:
            m.columnas_referencia = cols_legado
        else:
            m.columnas_referencia = []
        m.aberturas_globales = data.get("aberturas_globales", [])
        m.juntas_dilatacion = data.get("juntas_dilatacion", [])
        m.interfaces_junta = data.get("interfaces_junta", [])
        m.pendientes = data.get("pendientes", [])
        m.validacion = data.get("validacion", {})
        ...
        if not ids:
            ...
            for mu in m.muros:
                if mu.get("recibe_losa", False) and mu["id"] in catalogo:
                    ...
                    if r.linea.intersects(panel.exclusivo) or r.linea.within(panel.exclusivo):
                        ids.append(mu["id"])
            ids = list(dict.fromkeys(ids))  # deduplicar el fallback por orden

        # referencias rotas: a un ID declarado en apoyos_validos que no existe
        # en el catalogo NO se lo descarta silenciosamente; se reporta ValueError.
        faltantes = [rid for rid in panel.apoyos_validos if rid not in catalogo]
        if faltantes:
            raise ValueError(
                f"Losa '{panel.id}': apoyos_validos referencian receptores "
                f"inexistentes: {sorted(set(faltantes))}")

        # deduplicar manteniendo orden; ya se garantizo que todos existen
        seen = set()
        receptores = []
        for rid in ids:
            if rid in seen:
