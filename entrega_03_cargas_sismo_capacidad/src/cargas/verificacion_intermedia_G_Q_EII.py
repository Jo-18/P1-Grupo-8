"""Verificacion intermedia de superposicion G + Q del Edificio II (Entrega 3).

Replica sobre el Edificio II la verificacion INTERMEDIA ya hecha para el EI,
usando EXACTAMENTE el mismo modelo congelado `G_EII_REPRODUCIBLE_CON_DISCREPANCIAS_DOCUMENTADAS`
(reconstruido por `pipeline_FE_EII.MarcoEII`, escenario V.60/80):

  1. corrida G          : PP + losa (peso propio), sin SC          -> F_total_G.
  2. corrida Q          : SOBRECARGA uniforme q_Q sola, G ausente  -> F_total_Q.
  3. corrida EXPLICITA  : G + lambda_Q*Q en un UNICO patron de carga.

y compara la SUPERPOSICION  R_sup = lambda_G*G + lambda_Q*Q  contra la corrida
...
Controles previos (ABORTAN si fallan):
  * compatibilidad de los tres casos base (nodos, elementos, restricciones,
    DOF, claves y convenciones de resultados).
  * las cargas Q del patron explicito coinciden con la corrida Q separada.

Salidas:
  results/superposicion/verificacion_intermedia_G_Q_EII.json/csv/md
  figures/superposicion/verificacion_intermedia_G_Q_EII.png
...
la verificacion FINAL (G+Q+EX+EY) sigue BLOQUEADO_POR_PARAMETROS_SISMICOS.
"""
...
from src.cargas.pipeline_FE_EII import MarcoEII, ORDEN_NIVELES  # noqa: E402
from src.cargas.caso_Q_EII import construir_y_resolver, q_efectivo  # noqa: E402
...
def _cargas_G(marco) -> dict:
    """PP de elementos + losa (G), mismo flujo que el caso G congelado."""
    pp = marco.peso_propio_nodal()
    losa, _rep = marco.losa_tributaria_nodal(incluir_sc=False)
    cargas = {}
    for agg in (pp, losa):
...
def _cargas_Q(marco, q_Q) -> dict:
    """Sobrecarga uniforme q_Q sola (reparto tributario identico al caso G)."""
...
def _corrida_Q(info_q):
    """Reutiliza el caso Q verificado (caso_Q_EII): mismo modelo, G ausente."""
...
def _corrida_explicita(info_q):
    ...
    for t in set(G) | set(Q):
        a = G.get(t, [0.0] * 6)
        b = Q.get(t, [0.0] * 6)
        exp[t] = [a[i] + lam["Q"] * b[i] for i in range(6)]
    sol = marco.resolver_caso({"EII": exp})
    return marco, sol, exp, G, Q


def _firma(marco) -> dict:
    """Firma determinista del modelo EII congelado (nodos, elementos, restr.)."""
    e_rec = []
    ...
    return {"n_nodos": len(marco.nodes),
            ...
            "v031_seccion": marco.v031_seccion,
            "E_mpa": marco.E_mpa}
...
def _compatibilidad(sols, cargas_q_corrida, cargas_q_explicita, firmas) -> dict:
    ...
    max_dif = 0.0
    for t in set(cargas_q_corrida) | set(cargas_q_explicita):
        va = cargas_q_corrida.get(t, [0.0] * 6)
        vb = cargas_q_explicita.get(t, [0.0] * 6)
        for i in range(6):
...
def _seleccion(R, marco) -> list:
    ...
    for i_ext in (0, 6):
        raw.append((["axial", e_ax, i_ext, "N_i" if i_ext == 0 else "N_j", "kN"],
                    ...
                    "localForce OpenSees indice %d y %d" % (i_ext, i_ext + 6)))
    ...
    if best:
        ...
        raw.append((["momento", e, i, _nom_mom(i), "kN_m"],
                    ...
                    "localForce indice %d; Mz/My en extremo i/j" % i))
...
def _escribir(payload, filas, eq, max_err, lam, info_q, compat):
    ...
    jp = RES_DIR / "verificacion_intermedia_G_Q_EII.json"
    jp.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n",
                  encoding="utf-8")

    cp = RES_DIR / "verificacion_intermedia_G_Q_EII.csv"
    ...
    lineas = []
    lineas.append("# Verificacion intermedia de superposicion G + Q - Edificio II")
    lineas.append("")
    lineas.append("R_sup = lambda_G*G + lambda_Q*Q (lambda_G=%.3g, lambda_Q=%.3g, "
                  ...
                  "mismo modelo lineal congelado (G_EII_REPRODUCIBLE_CON_"
                  "DISCREPANCIAS_DOCUMENTADAS)." % (lam["G"], lam["Q"], lam["etiqueta"]))
    lineas.append("")
    ...
    lineas.append("")
    lineas.append("| Familia | Descripcion | Convencion | G | Q | Superpuesto | "
                  "Explicito | |Err| | Err.rel | Tol | Estado |")
    lineas.append("|---|---|---|---|---|---|---|---|---|---|---|")
    for r in filas:
    ...
    lineas.append("- momentos: residuo Mx=%.3f, My=%.3f kN*m, %s"
    ...
    lineas.append("")
    lineas.append("- firmas nodos/elementos/restricciones/DOF identicas: %s"
    ...
    lineas.append("")
    lineas.append("- **%s**"
                  % ("IMPLEMENTADO_Y_VERIFICADO_INTERMEDIO_II" if ok_all
                     else "ERROR_VERIFICACION"))
    lineas.append("- notas: verificacion intermedia con los casos base G y Q "
                  "compatibles sobre el modelo congelado EII; la verificacion "
                  "FINAL (G+Q+EX+EY) permanece BLOQUEADO_POR_PARAMETROS_SISMICOS.")
    lineas.append("")
    mp = RES_DIR / "verificacion_intermedia_G_Q_EII.md"
    mp.write_text("\n".join(lineas) + "\n", encoding="utf-8")
    return jp, cp, mp
...
def _figura(filas, lam):
    ...
    ax1.set_title("Superposicion vs corrida explicita (mismo modelo congelado)")
    ...
    ax2.grid(alpha=0.3)
    fig.suptitle("VERIFICACION_INTERMEDIA_G_Q_EII - Edificio II (congelado)",
                 fontsize=11)
    fig.tight_layout(rect=(0, 0, 1, 0.95))
    fp = FIG_DIR / "verificacion_intermedia_G_Q_EII.png"
...
def main(argv=None) -> int:
    ...
    info_q = q_efectivo(json.loads(CONFIG_CARGAS.read_text(encoding="utf-8")),
                        usar_demo, q_cli)
    lam = _lambdas()
    ...
    if not (sol_g["ok"] and sol_q["ok"] and sol_e["ok"]):
        raise SystemExit(
            "ABORTE: alguna corrida fallo. G=%s Q=%s EXPLICITA=%s"
    ...
    payload = {
        "verificacion": "intermedia_G_Q_EII",
        "edificio": "II",
        "etiqueta": "IMPLEMENTADO_Y_VERIFICADO_INTERMEDIO_II",
        "modelo_base": "G_EII_REPRODUCIBLE_CON_DISCREPANCIAS_DOCUMENTADAS",
    ...
    def _redondeados(s, cargas):
        return {"desplazamientos": {str(k): [round(float(x), 9) for x in v]
                ...
                "cargas_nodales": {str(k): [round(float(x), 6) for x in v]
    ...
    print(json.dumps({
        "verificacion": "intermedia_G_Q_EII",
        "estado": "IMPLEMENTADO_Y_VERIFICADO_INTERMEDIO_II" if ok_all
                  else "ERROR_VERIFICACION",
        "lambda_G": lam["G"], "lambda_Q": lam["Q"],
        "caso_Q": info_q["etiqueta"],
        "F_total_kN": payload["F_total_kN"],
        "compatibilidad": {"firmas_identicas": True,
                           "cargas_Q_consistentes":
                               compat["cargas_Q_consistentes"]},
