...
| `Q` — `q_Q` **basado en norma** (NCh 1537:2009 Tabla 4) | `PARAMETRO_BASADO_EN_NORMA_NCH1537_2009_TABLA4` | **q_Q = 3,0 kN/m²** para EI y EII, mínimo de la categoría "Escuelas · salas de clases" (antes `null`, luego adopción del grupo de 2,0 kN/m² reemplazada por la norma). EI verificación `ΣQ=q_Q·A=12.328,42 kN` (Δ0,0), EII `7.889,14 kN` (rel 5,7e-09) |
| `Q` — verificación `ΣQ = q_Q·A` por nivel y global | `IMPLEMENTADO_Y_VERIFICADO` | EI `OK` (0,0), EII `OK` (rel 9e-08); tol 0,005 |
| `Q` — **caso FE completo del Edificio I** (G ausente, equilibrio) | `IMPLEMENTADO_Y_VERIFICADO` | `src/cargas/caso_Q_EI.py` con q_Q=3,0: `ΣQ=q_Q·A=12.328,42 kN` (Δ0,0), `G_ausente`, equilibrio `Rz=Pz`; 324 nodos/349 elementos |
| `Q` — **caso FE completo del Edificio II** (G ausente, equilibrio) | `IMPLEMENTADO_Y_VERIFICADO` | `src/cargas/caso_Q_EII.py` sobre el modelo congelado EII: `ΣQ=q_Q·A=7.889,14 kN`, conservación exacta (Δ0,0); 222 nodos/264 elementos; `caso_Q_EII_FE.json` |
...
# Q: caso FE completo por edificio (q_Q=3,0 NCh 1537:2009 Tabla 4; sin --demo)
...
- **q_Q = 3,0 kN/m²**: la sobrecarga de uso se fija como el **mínimo** de la
  categoría "Escuelas · salas de clases" de la **Tabla 4 de la NCh 1537:2009**
  (clasificación `PARAMETRO_BASADO_EN_NORMA_NCH1537_2009_TABLA4`), documentado en
  `config/cargas.json` y `case_Q_*`; reemplaza la adopción previa del grupo de
  2,0 kN/m² y ya no requiere `--demo`.
